...
<p class="texto">SELECCIONE EL USUARIO </p> 
...
<!-- MESA -->
 ...
 <?php
        ...
        $query = $db->query("SELECT * FROM usuario INNER JOIN permisos ON permisos.`usuario_idUsuario`=usuario.`idUsuario` INNER JOIN rol ON rol.`idRol`=permisos.`rol_idRol` WHERE idRol='11'");
        if($query->num_rows > 0){ 
  ...
  <div id="products">
        ...
        <form method="post" id="form1" action="listado_venta_usuario.php"> 
        <figure id="photo" title="<?php echo $row["Nombre_usuario"];?>">
                <button type="submit" class="btn btn-link" name="idUsuario" id="idUsuario" value="<?php echo $row["idUsuario"]; ?>"><img src="images/usuario.png" width="200px"> 
                </button>
        </figure>                  
